...
const SettingRadioGroup = ({ label, options, selectedValue, onChange, disabled, iconType, disabledOptions = [] }: {
    label: string;
    options: Option[];
                    ...
                    return (
                        <div key={option.id}>
                            ...
                            <label
                                ...
                                title={isOptionDisabled && iconType === 'layerHeight' ? t('tooltip_layer_height_invalid_for_nozzle') : displayName}
...
const PrintOptions: React.FC<PrintOptionsProps> = ({
  ...
  const { availableLayerHeights, disabledLayerHeightValues } = useMemo(() => {
    const selectedNozzle = nozzles.find(n => n.id === selectedNozzleId);
    if (!selectedNozzle) {
        return { availableLayerHeights: [], disabledLayerHeightValues: layerHeights.map(lh => lh.value) };
    }
    ...
    const available = layerHeights.filter(lh => lh.value >= minLayerHeight && lh.value <= maxLayerHeight);
    const disabled = layerHeights.filter(lh => lh.value < minLayerHeight || lh.value > maxLayerHeight).map(lh => lh.value);

    return { availableLayerHeights: available, disabledLayerHeightValues: disabled };
  }, [selectedNozzleId, nozzles, layerHeights]);

  // Effect to auto-select a valid layer height when the nozzle changes
  useEffect(() => {
    if (disabledLayerHeightValues.includes(selectedLayerHeight)) {
        if (availableLayerHeights.length > 0) {
            // Find the closest valid option to the previously selected one, or default to the first valid one
            const sorted = [...availableLayerHeights].sort((a, b) => Math.abs(a.value - selectedLayerHeight) - Math.abs(b.value - selectedLayerHeight));
            onLayerHeightChange(sorted[0].value);
        }
    }
  }, [selectedNozzleId, disabledLayerHeightValues, availableLayerHeights, selectedLayerHeight, onLayerHeightChange]);
  ...
  return (
    <div className={`space-y-6 transition-opacity duration-500 ${disabled ? 'opacity-50' : 'opacity-100'}`}>
      ...
      <SettingRadioGroup
        ...
        disabledOptions={disabledLayerHeightValues}
      />
